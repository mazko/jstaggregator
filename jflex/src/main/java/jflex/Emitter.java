...
/**
 * This class manages the actual code generation, putting the scanner together,
 * filling in skeleton sections etc.
 * 
 * Table compression, String packing etc. is also done here.
 * 
 * @author Gerwin Klein
 * @version JFlex 1.5, $Revision: 614 $, $Date: 2012-08-31 16:47:28 +0300 (Птн,
 *          31 Авг 2012) $
 */
...
final public class Emitter {
	...
	// table[i][j] is the set of input characters that leads from state i to
	// state j
	private CharSet table[][];

	private boolean isTransition[];

	// noTarget[i] is the set of input characters that have no target state in
	// state i
...
//	private String visibility = "public";
	...
	public Emitter(File inputFile, LexParse parser, DFA dfa) throws IOException {
		...
		if (!"public".equals(scanner.visibility))
			throw new UnsupportedOperationException(
					"JavaScript target does not support '" + scanner.visibility
							+ "' visibility scope");
//		this.visibility = scanner.visibility;
	...
	/**
	 * Computes base name of the class name. Needs to take into account
	 * generics.
	 * 
	 * @param className
	...
	/**
	 * Constructs a file in Options.getDir() or in the same directory as another
	 * file. Makes a backup if the file already exists.
	 * 
	 * @param name
	 *            the name (without path) of the file
	 * @param input
	 *            fall back location if path = <tt>null</tt> (expected to be a
	...
	public static File normalize(String name, File input) {
		...
		if (outputFile.exists() && !Options.no_backup) {
			...
			if (outputFile.renameTo(backup))
				Out.println("Old file \"" + outputFile + "\" saved as \""
						+ backup + "\"");
			else
				Out.println("Couldn't save old file \"" + outputFile
						+ "\", overwriting!");
		}
	...
	private void emitLookBuffer() {
		...
		throw new UnsupportedOperationException(
				"JavaScript target does not support LookAhead");
...
//		println();
	...
	private void emitScanError() {
		if (scanner.scanErrorException != null)
			throw new UnsupportedOperationException(
					"JavaScript target does not support custom error types");

		println("  function zzScanError(errorCode)  {");
		...
		println("  function yypushback(number)  {");
...
//			println("    throw new Error(message);");
//		else
//			println("    throw new " + scanner.scanErrorException
//					+ "(message);");
//
//		skel.emitNext();
//
...
//			println(" throws " + scanner.scanErrorException + " {");
	...
	private void emitMain() {
		...
		throw new UnsupportedOperationException(
				"JavaScript target does not support standalone, debugOption, cupDebug options");
		
//		if (scanner.cupDebug) {
//			println("  /**");
//			println("   * Converts an int token code into the name of the");
//			println("   * token by reflection on the cup symbol class/interface "
//					+ scanner.cupSymbol);
//			println("   *");
...
//			println("    try {");
//			println("      java.lang.reflect.Field [] classFields = "
//					+ scanner.cupSymbol + ".class.getFields();");
...
//			println("");
//			println("  /**");
//			println("   * Same as " + scanner.functionName
//					+ " but also prints the token to standard out");
...
//		if (scanner.standalone) {
//			println("          while ( !scanner.zzAtEOF ) scanner."
//					+ scanner.functionName + "();");
//		} else if (scanner.cupDebug) {
//			println("          while ( !scanner.zzAtEOF ) scanner.debug_"
//					+ scanner.functionName + "();");
//		} else {
//			println("          do {");
//			println("            System.out.println(scanner."
//					+ scanner.functionName + "());");
...
//		println("            boolean eof = zzRefill();");
	...
	private void emitClassName() {
		if (!endsWithJavadoc(scanner.userCode)) {
			...
			println("/**");
			println(" * This function is a scanner generated by ");
			println(" * <a href=\"http://www.jflex.de/\">JFlex</a> "
					+ Main.version);
		...
		if (scanner.isPublic || scanner.isAbstract || scanner.isFinal)
			throw new UnsupportedOperationException(
					"JavaScript target does not support scanner Public, Abstract, Final, Extending, Implementing keywords");
...
//			println("  " + visibility + " static final int " + name + " = " + 2
//					* num + ";");
			println("  var " + name + " = " + 2 * num + ";");
		}

		// can't quite get rid of the indirection, even for non-bol lex states:
		// their DFA states might be the same, but their EOF actions might be
		// different
		// (see bug #1540228)
		println("");
		println("  /**");
...
//		println("      char value = packed.charAt(i++);");
		println("  function zzUnpackCMap(packed) {");
		println("    var map = new Array(0x10000);");
...
//		println("  private static final int ZZ_TRANS [] = {");
	...
	private void emitCharMapArrayUnPacked() {
		...
		throw new UnsupportedOperationException(
				"JavaScript target does not implement emitCharMapArrayUnPacked() method");
...
//		println("  private static final char [] ZZ_CMAP = zzUnpackCMap(ZZ_CMAP_PACKED);");
		println("  var ZZ_CMAP = zzUnpackCMap(ZZ_CMAP_PACKED);");
		println();
	}
	...
	private void emitConstructorDecl() {
		if (scanner.ctorArgs.size() > 0) throw new UnsupportedOperationException(
				"JavaScript target Ctor does not support additional args");
...
//
//		String warn = "// WARNING: this is a default constructor for "
...
//	}
	...
	private void emitDoEOF() {
		...
		throw new UnsupportedOperationException(
				"JavaScript target does not implement emitDoEOF() method");
...
//			// force public, because we have to implement
...
//		print("() throws java.io.IOException");
		...
		if (scanner.cupCompatible || scanner.tokenType != null
				|| scanner.isInteger || scanner.isIntWrap)
			throw new UnsupportedOperationException(
					"JavaScript target does not support cupCompatible and custom, int, IntWrap return type");
		
		print("  this." + scanner.functionName + " = function() ");
...
//		}
		...
		if (scanner.lexThrow != null || scanner.scanErrorException != null)
			throw new UnsupportedOperationException(
					"JavaScript target does not support custom throw and scan exeptions");
...
//			println("    int [] zzAttrL = ZZ_ATTRIBUTE;");
		...
		if (scanner.lineCount || scanner.columnCount) {
			throw new UnsupportedOperationException(
					"JavaScript target does not support lineCount and columnCount");
...
//			}
		...
		if (scanner.bolUsed) {
			throw new UnsupportedOperationException(
					"JavaScript target does not support bolUsed");
	...
	private void emitActions() {
		...
		for (Map.Entry<Action, Integer> entry : actionTable.entrySet()) {
			...
			if (action.lookAhead() == Action.FIXED_LOOK
				...
				throw new UnsupportedOperationException(
						"JavaScript target does not support lookAhead Action.FINITE_CHOICE");
...
//				println("          yypushback(" + action.getLookLength() + ");");
			...
			if (action.lookAhead() == Action.GENERAL_LOOK) {
				...
				throw new UnsupportedOperationException(
						"JavaScript target does not support lookAhead Action.GENERAL_LOOK");
				
//				println("          // general lookahead, find correct zzMarkedPos");
//				println("          { int zzFState = "
//						+ dfa.entryState[action.getEntryState()] + ";");
...
//				println();
//				println("            zzFState = "
//						+ dfa.entryState[action.getEntryState() + 1] + ";");
...
//				println("          }");
			...
			if (scanner.debugOption) {
				...
				throw new UnsupportedOperationException(
						"JavaScript target does not support debugOption");
...
//				print("          System.out.println(\"action ["
//						+ action.priority + "] { ");
...
//				println(" }\");");
	...
	private void emitEOFVal() {
		...
		if (scanner.eofCode != null)
			throw new UnsupportedOperationException(
					"JavaScript target does not support eofCode");
//			println("            zzDoEOF();");
...
//				println(" }\");");
			...
			throw new UnsupportedOperationException(
					"JavaScript target does not support eofVal");
//			println("              { " + scanner.eofVal + " }");
		else if (scanner.isInteger) {
			throw new UnsupportedOperationException(
					"JavaScript target does not support Integer return type");
//			if (scanner.tokenType != null) {
...
//			println("            return YYEOF;");
	...
	private int getDefaultTransition(int state) {
		...
		for (int i = 0; i < dfa.numStates; i++) {
			if (table[state][max] == null)
				max = i;
			else if (table[state][i] != null
					&& table[state][max].size() < table[state][i].size())
				max = i;
		}
	...
	private void setupEOFCode() {
		if (scanner.eofclose) {
			...
			scanner.eofThrow = LexScan.concExc(scanner.eofThrow,
					"java.io.IOException");
		}
	}
...
//		println("  private static final int ZZ_BUFFERSIZE = "
//				+ scanner.bufferSize + ";");
		println("  var ZZ_BUFFERSIZE = " + scanner.bufferSize + ";");
